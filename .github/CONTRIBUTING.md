...
links generated by Release Please remain intact. Credits for GitHub commit authors are added from the release's actual comparison
...
does not exist yet. The PR displays the same version/date, categories and contributor footer as the
published release. Original version details are retained in a hidden HTML comment
for Release Please publication; boilerplate and collapsed version controls are
not shown. Contributor profile names precede their linked handles, as in Payload
releases. The version heading links to the comparison.
