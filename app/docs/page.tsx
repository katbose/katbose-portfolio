...
1) Set up the project:
   ...
   - cd my-portfolio
   - bun install
   - bun run dev  (then tell me to open http://localhost:3000)
...
5) Verify:
   - run bun run build and fix any type error or missing field it reports
   - run bun run check and bun run typecheck (the pre-commit hook runs both)
    ...
    <div className="relative flex min-h-screen flex-col items-center bg-white dark:bg-black px-3 pt-16 text-black dark:text-white selection:bg-black dark:selection:bg-white selection:text-white dark:selection:text-black pb-32 sm:px-4 sm:pt-24 sm:pb-40 overflow-x-hidden transition-colors duration-300">
      ...
      <main className="flex w-full max-w-3xl flex-col text-left">
        ...
        <p className="mb-10 max-w-xl text-lg leading-relaxed text-gray-600 dark:text-gray-400">
          A JSON-driven personal portfolio built with Next.js. Write your content in one file, and
          the page builds itself. No component editing required.
        </p>
        ...
        <DocSection id="purpose" title="What is this">
          <p>
            Most portfolios mix content and code, so changing a job title means digging through JSX.
            Hackyfolio splits them apart. All your content lives in a single data file. A set of
            fixed components reads that file and renders the page.
          </p>
        ...
        <DocSection id="quick-start" title="Quick start">
          <p>
            You need <C>bun</C> installed. Clone the repo from{" "}
            <a
              href={REPO_URL}
              target="_blank"
              rel="noopener noreferrer"
...
# install dependencies (also wires up the git hooks)
bun install
...
bun run dev`}</CodeBlock>
          ...
          <CodeBlock label="terminal">{`bun run build
bun run start`}</CodeBlock>
          <p>
            Tip: the fastest way to fill this in is to hand your resume to an AI coding agent like
            Claude Code and ask it to populate the data file. The repo includes a <C>CLAUDE.md</C>{" "}
            that teaches agents the schema.
          </p>
        </DocSection>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="text-gray-600 dark:text-gray-400">
                {[
                  ...
                  <tr
                    key={type}
                    className="border-b border-gray-100 dark:border-zinc-900 last:border-0"
                  >
  ...
  "Another paragraph after the list."
          ...
          <p>
            Inline formatting works inside any string: <C>**bold**</C> and <C>[text](url)</C>. That
            is the whole syntax.
          </p>
        </DocSection>
        ...
        <DocSection id="arrange" title="Rearrange or remove sections">
          ...
          <p>
            To remove a section, delete its block. To hide one temporarily, cut it and paste it back
            later. No code touched either way.
          </p>
        </DocSection>
...
4. Add a block with "type": "awards" to portfolio.json`}</CodeBlock>
          <p>
            ...
            hard to get wrong. Run <C>bun run build</C> to confirm.
          </p>
        </DocSection>

        <DocSection id="deploy" title="Deploy">
          <p>
            The smoothest host is{" "}
            <a
            ...
            , by the team behind Next.js. It is free for personal sites.
          </p>
